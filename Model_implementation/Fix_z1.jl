...
@objective(modelo,Min,w*z1+(1-w)*z2
)
...
#sum(μ[e,j]*value(X[e,s,j,t]) for (e,s,j,t) in eachindex(X)) +
#sum(μ[j,k]*value(Y[j,k,t]) for (j,k,t) in eachindex(Y)) +
#sum(μ[j,k]*value(S[j,k,t]) for j in origem+silos+rodo+ferro+hidro+1:origem+silos+rodo+ferro+hidro+portos,k in D_i, t in T if c[j,k] < dist_max_imp)

#sum(λ[e]*value(P[e,s,t]) for e in 1:origem for s in 1:safra for t in T)
...
#@constraint(
#modelo,
#sum(Y[j,k,t] for (j,k,t) in eachindex(Y) if j >= origem+silos+rodo+1 && j <= origem+silos+rodo+ferro)>=
#0.5*sum(Y[j,k,t] for (j,k,t) in eachindex(Y) if j >= origem+silos+1 && j <= origem+silos+rodo+ferro+hidro)
#)
